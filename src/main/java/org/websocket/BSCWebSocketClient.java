...
public class BSCWebSocketClient extends WebSocketClient {
    ...
    // TODO：没有免费的wss服务节点，使用量太大
    static String QUIKNODE_WSS_BSC = dotenv.get("QUIKNODE_WSS_BSC");
    static Connection connection = ConncetDB.getConnect();
    static final int RECONNECT_INTERVAL = 5000; // 初始重连间隔
    ...
    public BSCWebSocketClient(URI serverUri, Draft draft) {
        super(serverUri, draft);
    }

    public BSCWebSocketClient(URI serverURI) {
        super(serverURI);
    }

    public BSCWebSocketClient(URI serverUri, Map<String, String> httpHeaders) {
    ...
    public static void main(String[] args) throws URISyntaxException, InterruptedException {
        try {
            client = new BSCWebSocketClient(new URI(QUIKNODE_WSS_BSC));
            client.connect();
